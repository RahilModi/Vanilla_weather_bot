...
let CurrentWeather = response =>{
  if(response.query.results){
    ...
    let {text,temp, code, date} = resp.item.condition; // text = resp.item.condition.text
    return `Right now @ ${date.bold}, ${getPrefix(code)} ${text.toLowerCase().red.bold} in ${location.bold}. It is ${getFeel(temp)} at ${temp.red.bold} degrees Celsius.`
  }else{
    return `I don't seem to know about ${location.bold}, sorry about that :(`;
  }
};
...
let forecastWeather = (response,data)=>{
  if(response.query.results){
      ...
      return `${testConditions ? 'Yes' : 'No'}, ${getPrefix(getForecast.code,'future')} ${getForecast.text.bold} ${data.time} in ${location} and Max temperature : ${getForecast.high.bold}, Min temperature : ${getForecast.low.bold} degrees Celsius.`
    }else{
